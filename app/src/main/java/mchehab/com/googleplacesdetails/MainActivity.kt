...
import mchehab.com.googleplacesdetails.cluster.MarkerClusterItem
import mchehab.com.googleplacesdetails.cluster.MarkerClusterRenderer
import mchehab.com.googleplacesdetails.location.LocationHandler
import mchehab.com.googleplacesdetails.location.LocationResultListener
import mchehab.com.googleplacesdetails.placedetail.PlaceDetailsActivity

class MainActivity : AppCompatActivity(), OnMapReadyCallback, LocationResultListener, PlaceSelectedListener {
    ...
    private val placesList = mutableListOf<GooglePlace>()
    ...
    private fun setClusterClickListener() {
        clusterManager.setOnClusterClickListener { cluster ->
            ...
            val clusterPlaces = mutableListOf<GooglePlace>()
            cluster.items.forEach {
                list.add(it.title)
                for (place in placesList){
                    if (place.latLng.equals(it.latLng)){
                        clusterPlaces.add(place)
                    }
                }
            }
            ListViewDialog(this@MainActivity, clusterPlaces, this@MainActivity).showDialog()
            true
        }
    }
    ...
    override fun getPlace(place: GooglePlace) {
    ...
    private inner class PlaceRequest : AsyncTask<String, Int, JSONArray>() {
        ...
        override fun onPostExecute(result: JSONArray) {
            ...
            for (i in 0 until result.length()) {
                ...
                val googlePlace = GooglePlace(name, latLng, placeId)
